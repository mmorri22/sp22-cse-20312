...
INTRO_MESSAGE="Grading script for Spring 2022 - Programming Challenge 02"
...
CHECK_EXE="test -f exe/PC02"    

# Next, we check for the object files  
if test -f "objects/PC02.o" || test -f "objects/life.o"; then
...
if test -f "exe/PC02"; then
    echo "PC02 Executable Exists." >> $SCRIPT_OUT
	echo "Executables must be cleaned prior to GitHub push." >> $SCRIPT_OUT
	echo "Automatic 50 point deduction, per project description" >> $SCRIPT_OUT
	echo "0 / $CHECK_EXE_PTS" >> $SCRIPT_OUT
	echo "Here are the files currently in the project folder." >> $SCRIPT_OUT
	CHECK_PC02_TEST_RESULT=$( $LIST_OUTPUTS ) >> $SCRIPT_OUT
	echo "$CHECK_PC02_TEST_RESULT" >> $SCRIPT_OUT
...
MAKE_PC02_CLEAN_FORCE=$( $MAKE_CLEAN_FORCE )
MAKE_PC02_TEST_RESULT=$( $MAKE_PC02_TEST )

# Delete redirect and send make result to redirect 
echo $MAKE_PC02_CLEAN_FORCE >> $REDIRECT
echo $MAKE_PC02_TEST_RESULT >> $REDIRECT

# Send results to the script
echo "$MAKE_PC02_TEST output  : " >> $SCRIPT_OUT
echo $MAKE_PC02_TEST_RESULT >> $SCRIPT_OUT
echo "" >> $SCRIPT_OUT
...
then 
	echo "$MAKE_PC02_TEST test passed" >> $SCRIPT_OUT
	...
	echo "$PC_02_PQC_TEST / $PC_02_PQC_TEST" >> $SCRIPT_OUT
	echo "" >> $SCRIPT_OUT
	((STUDENT_GRADE += $PC_02_PQC_TEST))
	
else
	echo "$MAKE_PC02_TEST test failed" >> $SCRIPT_OUT
	echo "0 / $PC_02_PQC_TEST" >> $SCRIPT_OUT
	echo "To the grading TA - Manually check if the make PC02 does not contain these flags." >> $SCRIPT_OUT
...
if grep -e "all warnings being treated as errors" $REDIRECT; then
    ...
    echo "PC02 did not successfully compile." >> $SCRIPT_OUT
	echo "Executables must be cleaned prior to GitHub push." >> $SCRIPT_OUT
	echo "Automatic 50 point deduction, per project description" >> $SCRIPT_OUT
	...
	echo "PC02 Successfully Compiled!" >> $SCRIPT_OUT
...
echo "-----------------------------" >> $SCRIPT_OUT
echo "" >> $SCRIPT_OUT
echo "PC02 Standards Grade Portion for $STUDENT_NAME ($STUDENT_ID): $STUDENT_GRADE / $TOTAL " >> $SCRIPT_OUT
echo "PC02 Standards Grade Portion for $STUDENT_NAME ($STUDENT_ID): $STUDENT_GRADE / $TOTAL "
