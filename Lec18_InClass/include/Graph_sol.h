...
struct Graph{
	...
	public:
		...
		Graph<T> MST(){
			...
			PRIORITY_QUEUE< MSTElem, VECTOR< MSTElem >, std::greater< MSTElem > > MST_PQ;
			...
			// Keep track of the visited vertices
			VECTOR<bool> visited(vertices.size(), -1 );
			

			// Keep track of the visited vertices
			VECTOR<int> weights(vertices.size(), 2147483647 );
			weights.at(0) = -2147483648;
