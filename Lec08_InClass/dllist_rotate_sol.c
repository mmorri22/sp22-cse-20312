...
void rotate_list( dllist* the_list, int num_rotations, char direction, int array_len ){
	
	// Mod to get the number of rotations
	num_rotations %= array_len;
	...
	the_list->head_node->prev_node = the_list->tail_node;
	the_list->tail_node->next_node = the_list->head_node;
	...
	if( direction == 'L' ){
		...
		for( iter = 0; iter < num_rotations; ++iter ){
			
			the_list->head_node = the_list->head_node->next_node;
			the_list->tail_node = the_list->tail_node->next_node;
		}
	}
	...
	// Iterate right
		...
		for( iter = 0; iter < num_rotations; ++iter ){
			...
			the_list->head_node = the_list->head_node->prev_node;
			the_list->tail_node = the_list->tail_node->prev_node;
...
int main( const int argc, const char* argv[] ){
	...
	fprintf( stdout, "Enter the number of rotations and direction: " );
	int num_rotations;
	char direction;
	
	fscanf( stdin, "%d %c", &num_rotations, &direction );
	
	rotate_list( the_list, num_rotations, direction, argc - 1 );
	
	fprintf( stdout, "Updated list:\n" );
	print_list( the_list->head_node );
