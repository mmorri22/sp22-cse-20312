...
/* modified with permission by Matthew Morrison <matt.morrison@nd.edu> 	    */
 ...
 */
long unsigned int lprofile[MAX_HEIGHT];
long unsigned int rprofile[MAX_HEIGHT];
...
long unsigned int min (long unsigned int X, long unsigned int Y)  {
	return ((X) < (Y)) ? (X) : (Y);
}

long unsigned int max (long unsigned int X, long unsigned int Y)  {
	return ((X) > (Y)) ? (X) : (Y);
}
...
struct pnode_struct {
    Pnode * left, * right;
    long unsigned int edge_length; /* length of the edge from this node to its children */
                     /* number of "\" or "/".  so it's at least 1         */
                     /* unless both children are null.  Then it's 0       */
    
    long unsigned int height;      /* The number of rows required to print this tree */
    long unsigned int lablen;
 ...
 */
void compute_lprofile(Pnode *pn, long unsigned int x, long unsigned int y) {
    long unsigned int i, isleft;
    if (pn == NULL) 
		return;
...
void compute_rprofile(Pnode *pn, long unsigned int x, long unsigned int y) {
    long unsigned int i, notleft;
...
void compute_edge_lengths(Pnode *pn) {
	...
	long unsigned int h, hmin, i, delta;
...
long unsigned int print_next;  /* used by print_level.  If you call "fprintf( stdout, )" at   */
 ...
 */
void print_level(Pnode *pn, long unsigned int x, long unsigned int level) {
...
void pretty_print_tree(Tree * t) {
    ...
    if (proot->height >= MAX_HEIGHT) {
		fprintf( stdout, "(This tree is taller than %d, and may be drawn incorrectly.)\n", MAX_HEIGHT);
    }
    free_ptree(proot); 
}
...
/* Return a pointer to the resulting tree.    
    ...
    new_Tree = (Tree *) my_alloc (sizeof (Tree));
    new_Tree->key = i;
	
	// Dr. Morrison's Golden Rule of Pointers 
    if (t == NULL) {
		new_Tree->left = new_Tree->right = new_Tree->parent = NULL;
		size = 1;
		return new_Tree;
    }
    ...
    if (i < t->key) {
		...
		new_Tree->right = t;
		t->left = NULL;
		set_parent_left(new_Tree);
		set_parent_right(new_Tree);
		size ++;
		return new_Tree;
    } 
	else if (i > t->key) {
...
int main( void ) {
    ...
    while(TRUE) {
		fprintf( stdout, "Enter the number of nodes in the tree: ");
		N = -1;
		if (fgets(line, sizeof(line), stdin) == NULL) exit(1);
		sscanf(line,"%d", &N);
		if ((N<1) || (N > 200)) {
			fprintf( stdout, "Choose a number between 1 and 200.\n");
			continue;
		}
		break;
    }
    ...
    for (;;) {
		...
		fprintf( stdout, "Select a node to splay: ");
		...
		if ((sscanf(line, "%d", &i) == 1) && (i >= 0 && i < N)) {
			...
			if (strncasecmp(line, "quit", strlen("quit")) == 0) {
				...
				fprintf( stdout, "Choose a number in [%d, %d] or type \"quit\".\n", 0, N-1);
				continue;
			}
		}
		pretty_print_tree(root);
